...
test.serial('Persona#findAll', async t => {
  ...
  let lista = await personas.findAll();
...
test.serial('Persona#createOrUpdate - new', async t => {
  ...
  persona = await personas.createOrUpdate(nuevoPersona);
...
test.serial('Persona#createOrUpdate - update', async t => {
  ...
  persona = await personas.createOrUpdate(newData);
...
test.serial('Persona#findAll#filter#telefono', async t => {
  const { personas } = repositories;
  let lista = await personas.findAll({ telefono: '123456' });
...
test.serial('Persona#delete', async t => {
  const { personas } = repositories;
  let deleted = await personas.deleteItem(test.idUser);
