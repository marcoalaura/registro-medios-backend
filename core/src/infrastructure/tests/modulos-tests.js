...
test.serial('Modulo#findAll', async t => {
  const { modulos } = repositories;
  let lista = await modulos.findAll();
...
test.serial('Modulo#findById', async t => {
  ...
  let modulo = await modulos.findById(id);
...
test.serial('Modulo#createOrUpdate - new', async t => {
  ...
  let modulo = await modulos.createOrUpdate(nuevoModulo);
...
test.serial('Modulo#createOrUpdate - exists', async t => {
  ...
  let modulo = await modulos.createOrUpdate(newData);
...
test.serial('Modulo#findAll#filter', async t => {
  const { modulos } = repositories;
  let lista = await modulos.findAll({ label: 'Nuevo', estado: 'ACTIVO', visible: true });
...
test.serial('Modulo#findAll#filter - id_modulo', async t => {
  const { modulos } = repositories;
  let lista = await modulos.findAll({ id_modulo: 1 });
...
test.serial('Modulo#findAll#filter - id_modulo - null', async t => {
  const { modulos } = repositories;
  let lista = await modulos.findAll({ id_modulo: 0 });
...
test.serial('Modulo#delete', async t => {
  const { modulos } = repositories;
  let deleted = await modulos.deleteItem(test.idModulo);
